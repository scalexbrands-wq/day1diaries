...
export default function JobDetail() {
  ...
  useEffect(() => {
    if (user) {
      setForm(f => ({
        ...f,
        full_name: f.full_name || user.full_name || user.username || '',
  ...
  return (
    ...
    >
      ...
      <div className="site-card">
        ...
        {!user && (
          <div style={{ background: 'rgba(255,107,43,.06)', border: '1px solid rgba(255,107,43,.2)', borderRadius: 10, padding: '10px 16px', fontSize: 13, color: '#8C7B6E', marginBottom: 18 }}>
            <Link to="/login" style={{ color: '#FF6B2B', fontWeight: 600 }}>Sign in</Link> to track this application in your Jobs tab.
          </div>
        )}
        ...
        {status === 'sent' ? (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            {user && (
              <p style={{ color: '#FF6B2B', fontSize: 13.5, fontWeight: 600 }}>
                Track your application progress in <Link to="/careers" style={{ color: '#FF6B2B' }}>My Applications →</Link>
              </p>
            )}
