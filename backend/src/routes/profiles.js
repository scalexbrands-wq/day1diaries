...
router.post('/me/avatar', requireAuth, upload.single('image'), async (req, res) => {
  ...
  const url = await imageStorage.saveImage(`avatars/${req.cognitoSub}-${Date.now()}.${ext}`, req.file.buffer, req.file.mimetype, baseUrl, { maxWidth: 512, maxHeight: 512 })
...
router.post('/me/banner', requireAuth, upload.single('image'), async (req, res) => {
  ...
  const baseUrl = `${req.protocol}://${req.get('host')}`
  const ext = (req.file.mimetype.split('/')[1] || 'jpg').replace('jpeg', 'jpg')
  const url = await imageStorage.saveImage(`banners/${req.cognitoSub}-${Date.now()}.${ext}`, req.file.buffer, req.file.mimetype, baseUrl, { maxWidth: 1600, maxHeight: 600 })
